...
def build_cost_impact(wb):
    ...
    ws[f"G{sr}"].value = f"=SUM(G5:G{sr-1})"
    ws[f"G{sr}"].number_format = "€#,##0.00"
    ws[f"G{sr}"].font = Font(bold=True, size=11, name="Calibri", color=C_WHITE)
    ws[f"G{sr}"].fill = fill(C_NAVY)
    ws[f"G{sr}"].alignment = right()
    ws[f"G{sr}"].border = border_all()

    ws[f"H{sr}"].value = f"=SUM(H5:H{sr-1})"
    ws[f"H{sr}"].number_format = "€#,##0.00"
    ws[f"H{sr}"].font = Font(bold=True, size=11, name="Calibri", color=C_WHITE)
    ws[f"H{sr}"].fill = fill(C_NAVY)
    ws[f"H{sr}"].alignment = right()
    ws[f"H{sr}"].border = border_all()

    ws[f"I{sr}"].value = f"=SUM(I5:I{sr-1})"
    ws[f"I{sr}"].number_format = '+€#,##0.00;-€#,##0.00;€0.00'
    ws[f"I{sr}"].font = Font(bold=True, size=11, name="Calibri", color=C_WHITE)
    ws[f"I{sr}"].fill = fill(C_NAVY)
    ws[f"I{sr}"].alignment = right()
    ws[f"I{sr}"].border = border_all()
